...
void SCU_enablePD_disablePU(SCU_T *pSCU, unsigned char port, unsigned char pin){
	(SCU->SFSP)[port][pin]|=(1<<4); //disable pull up
	(SCU->SFSP)[port][pin]|=(1<<3); //enable pull down
}
